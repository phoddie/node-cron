...
const MONTH_CONSTRAINTS = Object.freeze([
	31,
...
const PARSE_DEFAULTS = Object.freeze(['0', '*', '*', '*', '*', '*']);
...
function CronTime(luxon) {
	...
	CT.prototype = {
		...
		_parseField: function (value, type, constraints) {
			...
			value = value.replace(/* RE_WILDCARDS */ /\*/g, `${low}-${high}`);

			// commas separate information, so split based on those
			const allRanges = value.split(',');

			for (let i = 0; i < allRanges.length; i++) {
				if (allRanges[i].match(/* RE_RANGE */ /^(\d+)(?:-(\d+))?(?:\/(\d+))?$/g)) {
					allRanges[i].replace(/* RE_RANGE */ /^(\d+)(?:-(\d+))?(?:\/(\d+))?$/g, ($0, lower, upper, step) => {
